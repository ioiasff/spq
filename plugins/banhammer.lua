...
local function kick_ban_res(extra, success, result)
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "kick" then
         if member_id == from_id then
            send_large_msg(receiver, "شما نمی توانید خود را اخراج کنید.")
			return
         end
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "شما نمی توانید مدیران و صاحب گروه را اخراج کنید.")
			return
         end
		 kick_user(member_id, chat_id)
      elseif get_cmd == 'ban' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
			send_large_msg(receiver, "شما نمی توانید مدیران و صاحب گروه را بن کنید.")
			return
        end
      ...
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از بن جهانی در امد.')
...
local support_id = msg.from.id
 if matches[1]:lower() == 'ایدی' and msg.to.type == "chat" or msg.to.type == "user" then
    ...
    if type(msg.reply_id) ~= "nil" then
      local print_name = user_print_name(msg.from):gsub("‮", "")
	  local name = print_name:gsub("_", "")
        savelog(msg.to.id, name.." ["..msg.from.id.."] used ایدی ")
  ...
  if matches[1]:lower() == 'بن' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "شما نمی توانید مدیران و صاحب گروه را بن کنید."
        end
...
if matches[1]:lower() == 'اخراج' then
	...
	elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "شما نمی توانید مدیران و صاحب گروه را اخراج کنید."
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
  ...
  if matches[1]:lower() == 'بن جهانی' and is_admin1(msg) then -- Global ban
  ...
  if matches[1]:lower() == 'انبن جهانی' then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'کاربر ['..user_id..' ] از بن جهانی در امد.'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
